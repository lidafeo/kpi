...
exports.pagePfu = function(req, res) {
    getObjPeriod().then(objPeriod => {
        ...
        res.render('page-pfu', {period: period,
...
exports.getReport = function(req, res) {
    ...
    getObjPeriod().then(objPeriod => {
        ...
        if(objPeriod && objPeriod.length > 0) {
            ...
            console.log(period.start_date, period.finish_date);
            period.date1 = dateModule.dateForOut(period.start_date);
            period.date2 = dateModule.dateForOut(period.finish_date);
        }
        DB.custom.forReportPFU(dateModule.dateForInput(period.start_date), dateModule.dateForInput(period.finish_date)).then(userValues => {
